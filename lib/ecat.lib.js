...
//~ const ecat = require('bindings')('zecat');
...
class ECAT extends EventEmitter{
	 ...
	 * */
	sdoRead(position, index, subindex, type, timeout, debug){
		const self = this;
		...
		return ecat.sdoRead(position, index, subindex, size, timeout, Number(debug));
	}
}
