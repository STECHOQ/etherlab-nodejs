...
class ECAT extends EventEmitter{
	constructor(slaveJSON, freq, doSortSlave = false){
		...
		self._dmnAddr2Idx = {};
		self._timer = 0n;
		self._emitData = undefined;
		self.isReady = false;

		if(slaveJSON && freq) self.init(slaveJSON, freq, doSortSlave);
	}
		...
		try{
			...
			ecat.start(async (...args) => {
				try{
					...
					if(self._emitData) _config.data = data;
					...
					if(_config.state != state){
						...
						if(!self.isReady && isOperational){
							self._emitData = ecat.getEmitData();
					...
					// if master is not in OP state or emit data is disabled
					// skip emitting data
					if(!isOperational || !self._emitData){
						// reset timer
						_cycle.timer = hrtime.bigint();
						self._timer = _cycle.timer;
						return;
	...
	/**
	 * Turn on or off data emitter
	 * @param {boolean} isOn - data emitter state
	 * */
	setEmitData(isOn = false){
		ecat.setEmitData(isOn);
	}
}
